/**
 * Application-wide constants
 * Centralized configuration for business rules and enums
 */

const {
  USER_ROLES,
  PUBLIC_REGISTRATION_ROLES,
  USER_STATUS,
  TASK_STATUS,
  TASK_TYPES,
  TASK_PRIORITY,
  SUBMISSION_TYPE,
  QA_STATUS,
  CLIENT_REVIEW_STATUS,
  PAYMENT_STATUS,
  NOTIFICATION_TYPES,
  NOTIFICATION_STATUS,
  NOTIFICATION_PRIORITY,
  REVIEW_TYPE,
  TASK_STATE_TRANSITIONS,
  TEAM_VISIBILITY,
  TEAM_JOIN_POLICY,
  TEAM_STATUS,
  TEAM_ROLE,
  TEAM_MEMBERSHIP_STATUS,
  TEAM_INVITATION_STATUS,
  TEAM_JOIN_REQUEST_STATUS,
  TEAM_ACTIVITY_TYPES,
  TEAM_NOTIFICATION_TYPES,
  PROJECT_STATUS,
  PROJECT_VISIBILITY,
  PROJECT_PARTICIPANT_ROLE,
  PROJECT_PARTICIPANT_STATUS,
  PROJECT_TASK_STATUS,
  PROJECT_MILESTONE_STATUS,
  PROJECT_ACTIVITY_TYPES,
  PROJECT_NOTIFICATION_TYPES,
  PROJECT_STATE_TRANSITIONS,
  PROJECT_TASK_TRANSITIONS,
  PROJECT_MILESTONE_TRANSITIONS,
  CONTRIBUTION_EVIDENCE_TYPES,
  CONTRIBUTION_VERIFICATION_LEVELS,
  CONTRIBUTION_EVIDENCE_STATUSES,
  CONTRIBUTION_EVIDENCE_ORIGINS,
  REPOSITORY_PROVIDERS,
  REPOSITORY_VERIFICATION_STATUSES,
  SHOWCASE_STATUSES,
  TEAM_OPENING_STATUS,
  COLLABORATION_REQUEST_STATUS,
  ORGANIZATION_MANAGEMENT_MODE,
  ORGANIZATION_ROLE,
  ORGANIZATION_MEMBERSHIP_STATUS,
  ORGANIZATION_INVITATION_STATUS,
  OPPORTUNITY_APPLICATION_MODE,
  NATIVE_APPLICATION_STAGE,
  APPLICATION_ACTIVITY_TYPE,
  NATIVE_APPLICATION_TRANSITIONS,
} = require("../contracts/domain");

// Business Rules
const BUSINESS_RULES = {
  PLATFORM_COMMISSION:
    parseFloat(process.env.PLATFORM_COMMISSION_PERCENTAGE) || 15,
  DEFAULT_REVISION_LIMIT: parseInt(process.env.DEFAULT_REVISION_LIMIT) || 2,
  QA_REVIEW_SLA_HOURS: parseInt(process.env.QA_REVIEW_SLA_HOURS) || 12,
  DISPUTE_RESOLUTION_SLA_HOURS:
    parseInt(process.env.DISPUTE_RESOLUTION_SLA_HOURS) || 48,
  DEADLINE_WARNING_HOURS: 24,
  REVISION_DEADLINE_EXTENSION_HOURS: 48,
  MISSED_DEADLINE_PENALTY: 5,
  SUSPENSION_THRESHOLD_MISSED_DEADLINES: 3,
  MIN_PERFORMANCE_SCORE: 0,
  MAX_PERFORMANCE_SCORE: 100,
  MIN_RATING: 1,
  MAX_RATING: 5,
  PASSWORD_MIN_LENGTH: 8,
  MAX_FILE_SIZE: parseInt(process.env.MAX_FILE_SIZE) || 10485760, // 10MB
  ALLOWED_FILE_TYPES: [
    "image/jpeg",
    "image/png",
    "image/gif",
    "application/pdf",
    "application/msword",
    "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
    "video/mp4",
    "video/quicktime",
    "application/zip",
  ],
};

// Assignment scoring weights
const ASSIGNMENT_WEIGHTS = {
  PERFORMANCE_SCORE: 0.4,
  SKILL_MATCH: 0.3,
  AVAILABILITY: 0.2,
  COMPLETION_RATE: 0.1,
};

// Performance score calculation weights
const PERFORMANCE_WEIGHTS = {
  ON_TIME_COMPLETION: 0.4,
  FIRST_TIME_APPROVAL: 0.3,
  CLIENT_SATISFACTION: 0.2,
  QA_PASS_RATE: 0.1,
};

// Error codes
const ERROR_CODES = {
  AUTH_REQUIRED: "AUTH_REQUIRED",
  FORBIDDEN: "FORBIDDEN",
  VALIDATION_FAILED: "VALIDATION_FAILED",
  CONFLICT: "CONFLICT",
  VALIDATION_ERROR: "VALIDATION_ERROR",
  AUTHENTICATION_ERROR: "AUTHENTICATION_ERROR",
  AUTHORIZATION_ERROR: "AUTHORIZATION_ERROR",
  NOT_FOUND: "NOT_FOUND",
  DUPLICATE_ERROR: "DUPLICATE_ERROR",
  DATABASE_UNAVAILABLE: "DATABASE_UNAVAILABLE",
  INVALID_STATE_TRANSITION: "INVALID_STATE_TRANSITION",
  TEAM_NOT_FOUND: "TEAM_NOT_FOUND",
  TEAM_SLUG_TAKEN: "TEAM_SLUG_TAKEN",
  TEAM_ACCESS_DENIED: "TEAM_ACCESS_DENIED",
  ALREADY_TEAM_MEMBER: "ALREADY_TEAM_MEMBER",
  NOT_TEAM_MEMBER: "NOT_TEAM_MEMBER",
  TEAM_INVITATION_EXISTS: "TEAM_INVITATION_EXISTS",
  TEAM_INVITATION_NOT_FOUND: "TEAM_INVITATION_NOT_FOUND",
  JOIN_REQUEST_EXISTS: "JOIN_REQUEST_EXISTS",
  JOIN_REQUEST_NOT_FOUND: "JOIN_REQUEST_NOT_FOUND",
  INVALID_TEAM_ROLE: "INVALID_TEAM_ROLE",
  OWNER_TRANSFER_REQUIRED: "OWNER_TRANSFER_REQUIRED",
  PROJECT_NOT_FOUND: "PROJECT_NOT_FOUND",
  PROJECT_SLUG_TAKEN: "PROJECT_SLUG_TAKEN",
  PROJECT_ACCESS_DENIED: "PROJECT_ACCESS_DENIED",
  PROJECT_PARTICIPANT_REQUIRED: "PROJECT_PARTICIPANT_REQUIRED",
  PROJECT_PARTICIPANT_EXISTS: "PROJECT_PARTICIPANT_EXISTS",
  PROJECT_PARTICIPANT_NOT_FOUND: "PROJECT_PARTICIPANT_NOT_FOUND",
  PROJECT_TASK_NOT_FOUND: "PROJECT_TASK_NOT_FOUND",
  PROJECT_MILESTONE_NOT_FOUND: "PROJECT_MILESTONE_NOT_FOUND",
  PROJECT_STALE_WRITE: "PROJECT_STALE_WRITE",
  INSUFFICIENT_FUNDS: "INSUFFICIENT_FUNDS",
  RATE_LIMIT_EXCEEDED: "RATE_LIMIT_EXCEEDED",
  FILE_UPLOAD_ERROR: "FILE_UPLOAD_ERROR",
  INTERNAL_SERVER_ERROR: "INTERNAL_SERVER_ERROR",
};

module.exports = {
  USER_ROLES,
  PUBLIC_REGISTRATION_ROLES,
  USER_STATUS,
  TASK_STATUS,
  TASK_TYPES,
  TASK_PRIORITY,
  SUBMISSION_TYPE,
  QA_STATUS,
  CLIENT_REVIEW_STATUS,
  PAYMENT_STATUS,
  NOTIFICATION_TYPES,
  NOTIFICATION_STATUS,
  NOTIFICATION_PRIORITY,
  REVIEW_TYPE,
  BUSINESS_RULES,
  TASK_STATE_TRANSITIONS,
  ASSIGNMENT_WEIGHTS,
  PERFORMANCE_WEIGHTS,
  ERROR_CODES,
  TEAM_VISIBILITY,
  TEAM_JOIN_POLICY,
  TEAM_STATUS,
  TEAM_ROLE,
  TEAM_MEMBERSHIP_STATUS,
  TEAM_INVITATION_STATUS,
  TEAM_JOIN_REQUEST_STATUS,
  TEAM_ACTIVITY_TYPES,
  TEAM_NOTIFICATION_TYPES,
  PROJECT_STATUS,
  PROJECT_VISIBILITY,
  PROJECT_PARTICIPANT_ROLE,
  PROJECT_PARTICIPANT_STATUS,
  PROJECT_TASK_STATUS,
  PROJECT_MILESTONE_STATUS,
  PROJECT_ACTIVITY_TYPES,
  PROJECT_NOTIFICATION_TYPES,
  PROJECT_STATE_TRANSITIONS,
  PROJECT_TASK_TRANSITIONS,
  PROJECT_MILESTONE_TRANSITIONS,
  CONTRIBUTION_EVIDENCE_TYPES,
  CONTRIBUTION_VERIFICATION_LEVELS,
  CONTRIBUTION_EVIDENCE_STATUSES,
  CONTRIBUTION_EVIDENCE_ORIGINS,
  REPOSITORY_PROVIDERS,
  REPOSITORY_VERIFICATION_STATUSES,
  SHOWCASE_STATUSES,
  TEAM_OPENING_STATUS,
  COLLABORATION_REQUEST_STATUS,
  ORGANIZATION_MANAGEMENT_MODE,
  ORGANIZATION_ROLE,
  ORGANIZATION_MEMBERSHIP_STATUS,
  ORGANIZATION_INVITATION_STATUS,
  OPPORTUNITY_APPLICATION_MODE,
  NATIVE_APPLICATION_STAGE,
  APPLICATION_ACTIVITY_TYPE,
  NATIVE_APPLICATION_TRANSITIONS,
};
