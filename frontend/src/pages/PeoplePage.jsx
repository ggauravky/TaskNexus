...
import { Briefcase as BriefcaseBusiness, ChevronLeft, ChevronRight, Filter, FolderKanban, Loader2, Search, Send, UsersRound } from "lucide-react";
...
const PeopleResults = ({ items, onCollaborate }) => items.length ? <div className="grid gap-4 xl:grid-cols-2">{items.map((person) => <PersonCard key={person.id} person={person} onCollaborate={onCollaborate} />)}</div> : <DiscoveryEmpty title="No developers match all selected filters." copy="Try removing one skill, role, or availability filter. Results are never broadened silently." />;
