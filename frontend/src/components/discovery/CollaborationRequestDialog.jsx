...
const CollaborationRequestDialog = ({ person, opening, hackathon, onClose, onSent }) => {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      else await api.post("/collaboration-requests", { recipientId: person.id, message, ...(hackathon ? { hackathonId: hackathon.id } : {}) });
  ...
  return <Dialog onClose={onClose} titleId="collaboration-dialog-title" descriptionId="collaboration-dialog-description" panelClassName="max-w-lg border-[#34343a] bg-[#0f1011] text-[#f7f8f8]">
    <form onSubmit={submit}>
      ...
      <div className="p-5 sm:p-6"><div className="rounded-lg border border-[#23252a] bg-[#141516] p-3 text-sm"><p className="text-xs text-[#62666d]">Context</p><p className="mt-1 text-[#d0d6e0]">{opening ? `${opening.team.name} · ${opening.title}` : hackathon ? hackathon.name : "General collaboration"}</p></div><label className="mt-5 block"><span className="team-label">Optional message</span><textarea autoFocus value={message} onChange={(event) => setMessage(event.target.value)} maxLength={500} className="team-input min-h-28 resize-y" placeholder="Share what you would like to build together." /><span className="mt-2 block text-right text-xs text-[#62666d]">{message.length}/500</span></label></div>
